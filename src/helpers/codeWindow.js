...
function showCode(curCode, corX = 0, corY = 0, textSz = 30, boxW = 500) {
  ...
  if(needLen) {
    fill('#4df0d6');
    curHigh = curHigh - getSpeed(curHigh, needHigh);
    curCode.curHigh=curHigh;

    curLen = curLen - getSpeed(curLen, needLen);
    curCode.curLen=curLen;

    rect(corX, corY+ (curHigh - 1) * lineH + textSz * 0.25, boxW, lineH * curLen);
  }
...
function showOutput(curOutp, corX = 0, corY = 0, textSz = 30, maxLine = 5, boxW=1000) {
  push();
